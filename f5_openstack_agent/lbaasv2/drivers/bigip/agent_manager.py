...
from f5_openstack_agent.lbaasv2.drivers.bigip.tunnels import fdb
from f5_openstack_agent.lbaasv2.drivers.bigip.tunnels import tunnel
...
class LbaasAgentManager(periodic_task.PeriodicTasks):  # b --> B
    ...
    def _setup_rpc(self):
        ...
        if not self.conf.f5_global_routed_mode:
            ...
            self.__tunnel_rpc = agent_rpc.PluginApi(topics.PLUGIN)
            ...
            self.tunnel_handler = \
                handler.TunnelHandler(self.__tunnel_rpc, self.__l2_pop_rpc,
                                      self.context)
            bigips = self.lbdriver.get_all_bigips()
            self.tunnel_handler.init_network_cache(bigips)
    ...
    def add_fdb_entries(self, context, fdb_entries, host=None):
        ...
        try:
            LOG.debug('received add_fdb_entries: %s host: %s'
                      % (fdb_entries, host))
            fdb.FdbBuilder.handle_fdbs(fdb_entries)
        except f5_ex.F5NeutronException as exc:
